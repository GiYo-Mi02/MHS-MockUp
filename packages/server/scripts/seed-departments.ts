...
async function seedSLAPolicies() {
  ...
  urgencies.forEach(urgency => {
    categories.forEach(category => {
      policies.push({
        ...
        expected_resolution_hours: urgency.hours[category as keyof typeof urgency.hours]
      });
    });
  });
  ...
  const existingKeys = new Set(
    existing?.map(p => `${p.category}:${p.urgency_level}`) || []
  );

  const toInsert = policies.filter(
    p => !existingKeys.has(`${p.category}:${p.urgency_level}`)
  );

...
import { requireRole, type TokenPayload } from '../services/auth.js'

const router = Router()

// GET /api/dashboards/:type - Get dashboard data by type (department or stats)
...
async function handleDepartmentQueue(req: Request, res: Response, user: TokenPayload) {
...
async function handleDepartmentStats(req: Request, res: Response, user: TokenPayload) {
  const departmentId = user.role === 'ADMIN' ? Number(req.query.departmentId) || null : user.departmentId || null
  if (!departmentId) {
    return res.status(400).json({ error: 'Missing departmentId' })
  }
